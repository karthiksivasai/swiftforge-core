...
comment on function public.import_master(text, text, jsonb) is
  'Reusable master CSV import (geo + catalog + party incl. customers/vendors/consignees/shippers/service_mappings): VALIDATE (dry-run, no writes) or COMMIT (atomic; row errors -> import_row_errors, unexpected error -> full rollback).';
