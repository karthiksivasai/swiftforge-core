...
function AwbImportPage() {
  ...
  const patch = (updates: Partial<AwbImportForm>) => setForm((current) => ({ ...current, ...updates }));
  ...
  const handleImport = () => {
    ...
    if (!form.fileName) return toast.error("Please select import file");
    toast.success(form.addToJobQueue ? "AWB import added to job queue" : "AWB import started");
  };
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4 p-4 md:p-6">
      ...
      <Card className="min-w-0 border p-4">
        <button
          type="button"
          onClick={() => toast.success("AWB import template download started")}
        ...
        <div className="grid gap-x-3 gap-y-2 lg:grid-cols-4">
          ...
          <FieldWrapper label="Weight" required>
            <Input value={form.weight} onChange={(event) => patch({ weight: event.target.value })} className="h-9" />
          </FieldWrapper>

          <FieldWrapper label="Pieces" required>
            <Input value={form.pieces} onChange={(event) => patch({ pieces: event.target.value })} className="h-9" />
          </FieldWrapper>
          <FieldWrapper label="Booking Date" required>
            <Input type="date" value={form.bookingDate} onChange={(event) => patch({ bookingDate: event.target.value })} className="h-9" />
          </FieldWrapper>
          <FieldWrapper label="Manifest No">
            <Input value={form.manifestNo} onChange={(event) => patch({ manifestNo: event.target.value })} className="h-9" />
          </FieldWrapper>
          <SelectField label="Type" value={form.type} placeholder="Select Type" options={["Out Going", "In Coming"]} onChange={(type) => patch({ type })} required />

          <SelectField label="Format" value={form.format} options={["Standard Format"]} onChange={(format) => patch({ format })} required />
          <CheckField label="Add Inscan" checked={form.addInscan} onChange={(addInscan) => patch({ addInscan })} />
          <CheckField label="Add to Pickup" checked={form.addToPickup} onChange={(addToPickup) => patch({ addToPickup })} />
          <CheckField label="Single Line Consignee Address" checked={form.singleLineConsigneeAddress} onChange={(singleLineConsigneeAddress) => patch({ singleLineConsigneeAddress })} />

          <FieldWrapper label="Scan Date">
            <Input type="date" value={form.scanDate} onChange={(event) => patch({ scanDate: event.target.value })} className="h-9" />
          </FieldWrapper>
          <FieldWrapper label="Scan Time">
            <Input type="time" value={form.scanTime} onChange={(event) => patch({ scanTime: event.target.value })} className="h-9" />
          </FieldWrapper>
          <SelectField label="Service Center" value={form.serviceCenter} placeholder="Select Service Center" options={serviceCenters} onChange={(serviceCenter) => patch({ serviceCenter })} required />
          <SelectField label="Exception" value={form.exception} placeholder="Select Exception" options={exceptions} onChange={(exception) => patch({ exception })} required />

          <CheckField label="Update Entry" checked={form.updateEntry} onChange={(updateEntry) => patch({ updateEntry })} />
          <CheckField label="Delete Entry" checked={form.deleteEntry} onChange={(deleteEntry) => patch({ deleteEntry })} />
