...
comment on function public.import_master(text, text, jsonb) is
  'Reusable master CSV import (geo + catalog + party incl. customers/vendors/consignees/shippers/service_mappings/vendor_contracts/local_branches): VALIDATE (dry-run, no writes) or COMMIT (atomic; row errors -> import_row_errors, unexpected error -> full rollback).';
...
-- EXTEND public.lookup (0017–0028) — branch + local-branch keys appended. — all prior keys kept verbatim; vendor + bank keys appended.
