...
} from "@/lib/masters/resources/productTypes";
import {
...
const SEED_DATA: Omit<ProductTypeRow, "id">[] = [
...
const SEED: ProductTypeRow[] = SEED_DATA.map((s, i) => ({ id: String(i + 1), ...s }));

const PAGE_SIZE = 10;

export const Route = createFileRoute("/master/sales/product-type")({
  head: () => ({
    meta: [
      { title: "Product Type — Master — Courier ERP" },
      { name: "description", content: "Manage the product type master." },
    ],
  }),
  component: ProductTypePage,
});

function emptyRow(): Omit<ProductTypeRow, "id"> {
  return { code: "", name: "" };
}

function rowToView(r: ProductTypeDbRow): ProductTypeRow {
  return { id: r.id, code: r.code, name: r.name, row_version: r.row_version };
}

function toRaw(form: Omit<ProductTypeRow, "id">) {
  return { code: form.code, name: form.name };
}

function ProductTypePage() {
  const { isAuthenticated: authed } = useAuth();
  const rc = useMasterResource(productTypesResource);
  const live = useMasterList(productTypesResource, { enabled: authed });
  const queryClient = useQueryClient();

  const [demoRows, setDemoRows] = useState<ProductTypeRow[]>(SEED);
  ...
  const openEdit = (row: ProductTypeRow) => {
    setEditing(row);
  ...
  const handleSave = async () => {
    ...
    if (authed) {
      ...
      try {
        if (editing) {
          const patch = productTypeUpdateSchema.parse(raw);
          ...
          const values = productTypeCreateSchema.parse(raw);
          await rc.create.mutateAsync(values);
          toast.success("Product type added");
        }
        setOpen(false);
      } catch (err) {
        toast.error(toErrorMessage(err, "Could not save product type"));
      } finally {
        setSaving(false);
      }
      return;
    ...
    try {
      if (editing) productTypeUpdateSchema.parse(raw);
      else productTypeCreateSchema.parse(raw);
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    if (authed) {
      try {
        ...
        toast.error(toErrorMessage(err, "Could not delete product type"));
      }
    } else {
  ...
  const handleRefresh = () => {
    ...
    setPage(1);
    if (authed)
      queryClient.invalidateQueries({ queryKey: masterKeys.all(productTypesResource.key) });
    ...
    <div className="flex w-full flex-col gap-5 px-4 py-6 md:px-8 md:py-8">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Product Type</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Product Type</h1>
        <p className="text-sm text-muted-foreground">
          Manage the product type master used to classify shipments.
        </p>
      </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteTarget?.name ? ` (${deleteTarget.name})` : ""} from the product type master.
