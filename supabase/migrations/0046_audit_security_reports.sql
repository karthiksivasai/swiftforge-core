-- ===========================================================================
-- 0046  audit & security reports — Phase 5 Milestone 5E
-- ---------------------------------------------------------------------------
-- Extends 5A–5D reporting with audit/security metadata + source entities.
-- Does NOT change public execute_report / validate_report_filters signatures.
-- Data: audit_logs, login_logs, sessions only. No new audit schema.
-- ===========================================================================
...
declare
  ...
  v_module text := nullif(btrim(coalesce(p_filters->>'module_slug','')),'');
  v_action text := nullif(btrim(coalesce(p_filters->>'action_type','')),'');
  v_entity text := nullif(btrim(coalesce(p_filters->>'entity_type','')),'');
  v_entity_id text := nullif(btrim(coalesce(p_filters->>'entity_id','')),'');
  v_ip text := nullif(btrim(coalesce(p_filters->>'ip_address','')),'');
  v_session_state text := nullif(btrim(coalesce(p_filters->>'session_state','')),'');
begin
  begin
  ...
  -- Audit / security presets (login_logs.event snapshots)
  if p_def.report_key = 'failed-login-attempts' and v_event is null then
    v_event := 'LOGIN_FAILED';
  elsif p_def.report_key = 'forced-logout-history' and v_event is null then
    v_event := 'FORCED_LOGOUT';
  elsif p_def.report_key = 'permission-change-report' and v_event is null then
    v_event := 'PERMISSION_CHANGE';
  end if;
  ...
  if p_def.source_entity = 'SHIPMENTS' then
    ...
    select count(*) into v_total
      ...
      left join public.users u on u.id = l.user_id and u.tenant_id = l.tenant_id
     ...
     where l.tenant_id = p_tenant
       ...
       and (
         case
           when p_def.report_key = 'authentication-activity' and v_event is null then
             l.event in ('LOGIN_SUCCESS','LOGIN_FAILED','LOGOUT','FORCED_LOGOUT')
           else (v_event is null or l.event = v_event)
         end
       )
       and (v_username is null or l.username ilike '%'||v_username||'%')
       and (v_ip is null or host(l.ip_address) ilike '%'||v_ip||'%')
       and (v_branch_code is null or b.code = v_branch_code);
  ...
  elsif p_def.source_entity = 'AUDIT_LOGS' then
    select count(*) into v_total
      from public.audit_logs a
      left join public.users u on u.auth_user_id = a.actor_id and u.tenant_id = a.tenant_id
      left join public.branches b on b.id = u.home_branch_id and b.tenant_id = u.tenant_id
     where a.tenant_id = p_tenant
       and a.created_at::date between p_from and p_to
       and (v_action is null or a.action = v_action)
       and (v_module is null or a.module_slug = v_module)
       and (v_entity is null or a.entity_type = v_entity)
       and (v_entity_id is null or a.entity_id::text = v_entity_id)
       and (v_username is null or u.username ilike '%'||v_username||'%')
       and (v_ip is null or host(a.ip_address) ilike '%'||v_ip||'%')
       and (v_branch_code is null or b.code = v_branch_code);
    ...
    select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) into v_rows
      from (
        ...
        where a.tenant_id = p_tenant
          ...
          and (v_branch_code is null or b.code = v_branch_code)
        order by
          case when v_dir = 'asc' then a.created_at end asc,
          case when v_dir = 'desc' then a.created_at end desc
        limit p_limit offset p_offset
      ) t;

  elsif p_def.source_entity = 'SESSIONS' then
    select count(*) into v_total
     ...
     where s.tenant_id = p_tenant
       ...
       and (v_username is null or u.username ilike '%'||v_username||'%')
       and (v_ip is null or host(s.ip_address) ilike '%'||v_ip||'%')
       and (v_branch_code is null or b.code = v_branch_code)
       and (
         v_session_state is null
         or (v_session_state = 'ACTIVE' and s.revoked_at is null
             and (s.expires_at is null or s.expires_at > now()))
         or (v_session_state = 'REVOKED' and s.revoked_at is not null)
         or (v_session_state = 'EXPIRED' and s.revoked_at is null
    ...
    select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) into v_rows
      from (
        ...
        order by
          case when v_dir = 'asc' then s.created_at end asc,
          case when v_dir = 'desc' then s.created_at end desc
        limit p_limit offset p_offset
      ) t;
...
-- ===========================================================================
-- Seed audit & security report definitions
-- ===========================================================================
do $$
declare
  ...
  v_audit_filters jsonb := jsonb_build_array(
    jsonb_build_object('key','username','label','User','type','TEXT'),
    jsonb_build_object('key','module_slug','label','Module','type','TEXT'),
    jsonb_build_object('key','action_type','label','Action Type','type','ENUM',
      'options', jsonb_build_array('ADD','MODIFY','DELETE','LOGIN','LOGOUT','ACCESS')),
    jsonb_build_object('key','entity_type','label','Entity','type','TEXT'),
    jsonb_build_object('key','entity_id','label','Entity Id','type','TEXT'),
    jsonb_build_object('key','ip_address','label','IP Address','type','TEXT'),
    jsonb_build_object('key','branch_code','label','Branch','type','TEXT')
  );
  v_login_filters jsonb := jsonb_build_array(
    jsonb_build_object('key','username','label','User','type','TEXT'),
    jsonb_build_object('key','event','label','Login Status','type','ENUM',
      'options', jsonb_build_array('LOGIN_SUCCESS','LOGIN_FAILED','LOGOUT','FORCED_LOGOUT','PERMISSION_CHANGE')),
    jsonb_build_object('key','ip_address','label','IP Address','type','TEXT'),
    jsonb_build_object('key','branch_code','label','Branch','type','TEXT')
  );
  v_audit_cols jsonb := jsonb_build_array(
    jsonb_build_object('key','created_at','label','At'),
    jsonb_build_object('key','action_type','label','Action'),
...
begin
  ...
  perform app.seed_report_definition(
    'action-log', 'AUDIT', 'Action Log',
    'Append-only audit trail across modules.',
    'rpt.action-log', 'AUDIT_LOGS', 'created_at',
    v_date || v_audit_filters, v_audit_cols, 400);

  perform app.seed_report_definition(
    'module-action-log', 'AUDIT', 'Module Action Log',
    'Audit trail filtered by module slug.',
    'rpt.action-log', 'AUDIT_LOGS', 'created_at',
    v_date || v_audit_filters, v_audit_cols, 410);

  perform app.seed_report_definition(
    'record-history-report', 'AUDIT', 'Record History Report',
    'Audit history for an entity type / id.',
    'rpt.action-log', 'AUDIT_LOGS', 'created_at',
    v_date || v_audit_filters, v_audit_cols, 420);

  perform app.seed_report_definition(
    'user-activity-report', 'AUDIT', 'User Activity Report',
    'Audit actions attributed to a user.',
    'rpt.user-analysis-report', 'AUDIT_LOGS', 'created_at',
    v_date || v_audit_filters, v_audit_cols, 430);

  perform app.seed_report_definition(
    'permission-change-report', 'AUDIT', 'Permission Change Report',
    'Permission grant changes (login_logs PERMISSION_CHANGE).',
    'rpt.action-log', 'LOGIN_LOGS', 'created_at',
    v_date || v_login_filters, v_login_cols, 440);
  ...
  perform app.seed_report_definition(
    ...
    v_date || v_login_filters, v_login_cols, 450);

  perform app.seed_report_definition(
    'failed-login-attempts', 'AUDIT', 'Failed Login Attempts',
    'Failed authentication attempts (defaults to LOGIN_FAILED).',
    'rpt.login-log', 'LOGIN_LOGS', 'created_at',
    v_date || v_login_filters, v_login_cols, 460);

  perform app.seed_report_definition(
    'session-activity', 'AUDIT', 'Session Activity',
    'Application session rows (active / revoked / expired).',
    'rpt.login-log', 'SESSIONS', 'created_at',
    v_date || jsonb_build_array(
      jsonb_build_object('key','username','label','User','type','TEXT'),
      jsonb_build_object('key','session_state','label','Session State','type','ENUM',
        'options', jsonb_build_array('ACTIVE','REVOKED','EXPIRED')),
      jsonb_build_object('key','ip_address','label','IP Address','type','TEXT'),
      jsonb_build_object('key','branch_code','label','Branch','type','TEXT')
    ),
    jsonb_build_array(
      jsonb_build_object('key','session_id','label','Session'),
      jsonb_build_object('key','username','label','User'),
      jsonb_build_object('key','app','label','App'),
      jsonb_build_object('key','ip_address','label','IP'),
      jsonb_build_object('key','branch_code','label','Branch'),
      jsonb_build_object('key','created_at','label','Started'),
      jsonb_build_object('key','last_seen_at','label','Last Seen'),
      jsonb_build_object('key','session_state','label','State'),
      jsonb_build_object('key','revoked_at','label','Revoked At'),
      jsonb_build_object('key','revoke_reason','label','Reason')
    ), 470);

  perform app.seed_report_definition(
    'forced-logout-history', 'AUDIT', 'Forced Logout History',
    'Forced logout events (defaults to FORCED_LOGOUT).',
    'rpt.login-log', 'LOGIN_LOGS', 'created_at',
    v_date || v_login_filters, v_login_cols, 480);

  perform app.seed_report_definition(
    'authentication-activity', 'AUDIT', 'Authentication Activity',
    'Login / logout / failed / forced-logout events.',
    'rpt.login-log', 'LOGIN_LOGS', 'created_at',
    v_date || v_login_filters, v_login_cols, 490);
...
comment on function app.execute_report_source is
  'Source-entity report executor (5A–5E). Audit/security: AUDIT_LOGS, LOGIN_LOGS, SESSIONS.';
