...
function ContentPage() {
  const [rows, setRows] = useState<ContentRow[]>(SEED);
  ...
  const handleSave = () => {
    if (!form.code.trim()) {
      toast.error("Content Code is required");
      return;
    }
    if (!form.name.trim()) {
  ...
  const handleExport = () => {
    const header = ["Content Code", "Content Name"];
    const csv = [
      header.join(","),
      ...rows.map((r) =>
        [r.code, r.name]
    ...
    toast.success("Exported contents.csv");
  };
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteTarget?.name ? ` (${deleteTarget.name})` : ""} from the content master. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
