...
function toFormField([name, type, enabled, value]: [string, string, boolean, string]): FormFieldConfig {
...
export const Route = createFileRoute("/utility/tax-charges-setup/setup")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Configure miscellaneous utility setup and module email controls." },
    ],
  }),
  component: SetupPage,
});
...
function SetupPage() {
  ...
  const [formFieldsByModule, setFormFieldsByModule] = useState<Record<FormSetupModuleKey, FormFieldConfig[]>>(buildFormSetupState);

  const activeConfig = useMemo(() => {
    if (!searchedModule || searchedModule === "KYC XPRESION" || searchedModule === "E-MAIL FOOTER") return null;
    return configs[searchedModule];
  }, [configs, searchedModule]);

  const patchConfig = (updates: Partial<EmailConfig>) => {
    if (!searchedModule || searchedModule === "KYC XPRESION" || searchedModule === "E-MAIL FOOTER") return;
    setConfigs((current) => ({
      ...current,
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4 p-4 md:p-6">
      ...
      <div className="flex items-center gap-2">
        <TabButton active={tab === "miscellaneous"} onClick={() => setTab("miscellaneous")}>MISCELLANEOUS</TabButton>
        <TabButton active={tab === "setup"} onClick={() => setTab("setup")}>SETUP</TabButton>
        <TabButton active={tab === "formSetup"} onClick={() => setTab("formSetup")}>FORM SETUP</TabButton>
      </div>

      {tab === "miscellaneous" ? (
        <div className="grid gap-4 lg:grid-cols-3">
          <MiniSaveCard title="LAST NO. SETUP" label="Invoice No." value={invoiceNo} onChange={setInvoiceNo} />
          <MiniSaveCard title="MISCELLANEOUS" label="Entry Lock Date" type="date" value={entryLockDate} onChange={setEntryLockDate} />
          <MiniSaveCard title="Credit Limit Email on %" label="Percentage" value={creditLimitPercentage} onChange={setCreditLimitPercentage} />
        </div>
      ) : null}
        ...
        <div className="flex flex-col gap-4">
          <Card className="w-full max-w-xl border p-4">
            <div className="flex flex-wrap items-end gap-8">
              <SelectField label="Module Name" value={moduleName} options={moduleOptions} onChange={(value) => setModuleName(value as ModuleKey)} />
              <Button onClick={() => setSearchedModule(moduleName)} className="h-9 rounded-full bg-slate-600 px-8 text-white hover:bg-slate-700">
                Search
              </Button>
            </div>
          </Card>
          ...
          {searchedModule ? (
            <Card className="w-full max-w-xl border p-4">
              {searchedModule === "KYC XPRESION" ? (
                ...
                <FooterSetup rows={footerRows} setRows={setFooterRows} onCancel={() => setSearchedModule(null)} />
              ) : activeConfig ? (
                <EmailSetup moduleName={searchedModule} config={activeConfig} patch={patchConfig} onCancel={() => setSearchedModule(null)} />
              ) : null}
            </Card>
          ) : null}
        </div>
        ...
        <FormFieldSetup
          ...
          setFields={(updater) => {
            setFormFieldsByModule((current) => ({
              ...current,
              [formModuleName]: typeof updater === "function" ? updater(current[formModuleName]) : updater,
            }));
          }}
        />
      ) : null}
...
function MiniSaveCard({ title, label, value, onChange, type = "text" }: { title: string; label: string; value: string; onChange: (value: string) => void; type?: string }) {
  return (
    <Card className="relative border p-4 pt-7">
      <span className="absolute -top-3 left-4 rounded-full bg-sidebar px-4 py-1 text-xs font-semibold text-sidebar-foreground shadow">{title}</span>
      <TextField label={label} value={value} onChange={onChange} type={type} required />
      <div className="mt-4 flex justify-center">
        <Button onClick={() => toast.success(`${title} saved`)} className="h-8 rounded-full bg-green-500 px-10 text-white hover:bg-green-600">Save</Button>
      </div>
    </Card>
  );
}

function EmailSetup({ moduleName, config, patch, onCancel }: { moduleName: ModuleKey; config: EmailConfig; patch: (updates: Partial<EmailConfig>) => void; onCancel: () => void }) {
  const forwardingFields = moduleName === "EMAIL ON FORWARDING" ? [
    ["Print DOX/SPX", "printDoxSpx"],
    ["Print PCS", "printPcs"],
    ["Print Weight", "printWeight"],
    ["Print Vendor Name", "printVendorName"],
  ] as const : [];
  const weightAlertFields = moduleName === "ESTATEMENT WEIGHT ALERT" ? [
  ...
  return (
    <div className="space-y-3">
      <HeaderRow />
      <TextField label="SMTP Server" value={config.smtpServer} onChange={(smtpServer) => patch({ smtpServer })} />
      <TextField label="SMTP Port" value={config.smtpPort} onChange={(smtpPort) => patch({ smtpPort })} />
      <TextField label="From Email" value={config.fromEmail} onChange={(fromEmail) => patch({ fromEmail })} />
      <TextField label="Mail User ID" value={config.mailUserId} onChange={(mailUserId) => patch({ mailUserId })} />
      <TextField label="Mail Password" value={config.mailPassword} onChange={(mailPassword) => patch({ mailPassword })} />
      <TextField label="Message Dear" value={config.messageDear} onChange={(messageDear) => patch({ messageDear })} />
      <TextField label="Message Subject" value={config.messageSubject} onChange={(messageSubject) => patch({ messageSubject })} />
      <TextField label="Message Body" value={config.messageBody} onChange={(messageBody) => patch({ messageBody })} />
      <CheckRow label="SSL" checked={config.ssl} onChange={(ssl) => patch({ ssl })} />
      {config.date !== undefined ? <TextField label="Date" value={config.date} onChange={(date) => patch({ date })} /> : null}
      {config.sendingTime !== undefined ? <TextField label="Sending Time" value={config.sendingTime} onChange={(sendingTime) => patch({ sendingTime })} /> : null}
      {[...forwardingFields, ...weightAlertFields].map(([label, key]) => (
        <CheckRow key={key} label={label} checked={Boolean(config[key])} onChange={(checked) => patch({ [key]: checked })} />
      ))}
      <ActionButtons onUpdate={() => toast.success(`${moduleName} updated`)} onCancel={onCancel} />
    </div>
  );
}

function FooterSetup({ rows, setRows, onCancel }: { rows: { controlName: string; code: string }[]; setRows: React.Dispatch<React.SetStateAction<{ controlName: string; code: string }[]>>; onCancel: () => void }) {
  return (
    <div className="space-y-3">
      <HeaderRow />
      {rows.map((row, index) => (
        <div key={row.controlName} className="grid grid-cols-[9rem_1fr] items-center gap-4 text-xs">
          ...
          <Input value={row.code} onChange={(event) => setRows((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, code: event.target.value } : item))} className="h-9" />
        </div>
      ))}
  ...
  return (
    <Card className="w-full max-w-5xl border p-4">
      <div className="mb-5 flex flex-wrap items-end gap-10">
        ...
        />
        <Button onClick={() => toast.success(`${moduleName} loaded`)} className="h-9 rounded-full bg-slate-600 px-8 text-white hover:bg-slate-700">
          Search
        </Button>
      </div>
      ...
      <div className="space-y-2 pr-2">
        {fields.map((field, index) => (
          <div key={field.name} className="grid grid-cols-[minmax(16rem,1fr)_minmax(12rem,1fr)] items-center gap-8 text-xs">
            <span>{field.name}</span>
            {field.type === "checkbox" ? (
              <Checkbox
                checked={field.enabled}
                onCheckedChange={(checked) => setFields((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, enabled: Boolean(checked) } : item))}
              />
            ) : (
              <Input
                value={field.value}
      ...
      <div className="mt-4 flex justify-end">
        <Button onClick={() => toast.success(`${moduleName} form setup saved`)} className="h-8 rounded-full bg-green-500 px-8 text-white hover:bg-green-600">Save</Button>
      </div>
    </Card>
  );
}
...
function ActionButtons({ onUpdate, onCancel }: { onUpdate: () => void; onCancel: () => void }) {
  return (
    <div className="flex justify-end gap-2 pt-2">
      <Button onClick={onUpdate} className="h-8 rounded-full bg-green-500 px-8 text-white hover:bg-green-600">Update</Button>
      <Button onClick={onCancel} className="h-8 rounded-full bg-red-500 px-8 text-white hover:bg-red-600">Cancel</Button>
    </div>
  );
}
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <Button type="button" size="sm" variant={active ? "default" : "ghost"} className={`h-8 rounded-full px-4 ${active ? "bg-sidebar text-sidebar-foreground hover:bg-sidebar/90" : "text-muted-foreground"}`} onClick={onClick}>
      {children}
    </Button>
  );
}
