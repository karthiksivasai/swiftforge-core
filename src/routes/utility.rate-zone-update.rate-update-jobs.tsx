...
export const Route = createFileRoute("/utility/rate-zone-update/rate-update-jobs")({
  head: () => ({
    meta: [
      { title: "Rate Update Jobs — Utility — Courier ERP" },
      { name: "description", content: "Bulk shipment rate recalculation jobs." },
    ],
  }),
  component: RateUpdateJobsPage,
});
...
function RateUpdateJobsPage() {
  ...
  const listQuery = useQuery({
    queryKey: ["rate-update-jobs", status, page],
    queryFn: () =>
      listRateUpdateJobs({
  ...
  const invalidate = () => {
    void queryClient.invalidateQueries({ queryKey: ["rate-update-jobs"] });
  };

  const executeMut = useMutation({
    mutationFn: (id: string) => executeRateUpdateJob(id),
    onSuccess: (job) => {
      toast.success(
  ...
  const cancelMut = useMutation({
    mutationFn: (id: string) => cancelRateUpdateJob(id),
    onSuccess: () => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-6 md:px-6 md:py-8">
      <MasterBreadcrumb trail={["Utility", "Rate / Zone Update", "Rate Update Jobs"]} />

      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">Rate Update Jobs</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">
            Recalculate shipment charge snapshots via the rating engine. Locked / invoiced /
            cancelled AWBs are skipped.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button type="button" variant="outline" size="sm" asChild>
            <Link to="/utility/rate-zone-update/rate-update">New job</Link>
          </Button>
        </div>
      </div>

      {!authed ? (
        <p className="text-sm text-muted-foreground">Sign in to view rate update jobs.</p>
      ) : listQuery.isError ? (
            ...
            <table className="w-full min-w-[760px] text-left text-sm">
              ...
              <tbody>
                {listQuery.isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="py-6 text-muted-foreground">
                      No jobs yet. Create one from Rate Update.
                    </td>
                  </tr>
                ) : (
                  rows.map((job) => (
                    <tr key={job.id} className="border-b last:border-0">
                      <td className="py-2.5 pr-3 font-medium">{job.update_type}</td>
                      <td className="py-2.5 pr-3">
                        <Badge variant="secondary" className="font-normal">
                          {rateUpdateJobStatusLabel(job.status)}
                        </Badge>
                      </td>
                      ...
                      <td className="py-2.5">
                        <div className="flex flex-wrap gap-1.5">
                          {canRetryRateUpdateJob(job.status) ? (
                            <Button
                              type="button"
                              size="sm"
                              variant="outline"
                          ...
                          {canCancelRateUpdateJob(job.status) ? (
                            <Button
                              type="button"
                              size="sm"
                              variant="outline"
